// ============================================================
// Cliente de datos en vivo: Binance (spot y futuros, sin clave)
// y OKX (liquidaciones reales). REST + WebSockets.
// ============================================================
...
// ---------- velas reales (spot o futuros) ----------
export async function fetchKlines(symbol: string, interval: string, limit: number, futures = true): Promise<Candle[]> {
  const base = futures ? REST_FUT : REST_SPOT;
...
export async function fetchFundingOi(symbol: string): Promise<FundingOi | null> {
  ...
  if (pf.status === "fulfilled" && pf.value.ok) {
    ...
    mark = Number(j.markPrice);
  }
  let oi = NaN;
  if (po.status === "fulfilled" && po.value.ok) {
    const j = (await po.value.json()) as { openInterest?: string };
    // openInterest viene en ACTIVO BASE → convertir a USD con el markPrice
    const qty = Number(j.openInterest);
    if (Number.isFinite(qty) && Number.isFinite(mark) && mark > 0) oi = qty * mark;
  }
...
// ---------- sentimiento real (ratio long/short) ----------
...
export function connectTickers(
  ...
  const open = () => {
    ...
    ws = new WebSocket((futures ? WS_FUT : WS_SPOT) + streams);
    ws.onopen = () => {
      retry = 0;
      everOpened = true;
    };
    ...
    ws.onclose = () => {
      if (!closed) {
        retry += 1;
        // si nunca abrió tras varios intentos (bloqueo regional), avisar
...
export function connectTrades(
  ...
  const open = () => {
    if (closed) return;
    ws = new WebSocket((futures ? WS_FUT : WS_SPOT) + `${symbol.toLowerCase()}@aggTrade`);
    ws.onopen = () => { retry = 0; };
    ws.onmessage = (ev) => {
      try {
