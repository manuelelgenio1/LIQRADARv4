// ============================================================
// Cliente de datos en vivo: Binance (PERP por defecto, SPOT como
// respaldo) y OKX (liquidaciones reales).
// ============================================================
...
const REST_FUT = "https://fapi.binance.com/fapi/v1";
const WS_SPOT = "wss://data-stream.binance.vision/stream?streams=";
const WS_FUT = "wss://fstream.binance.com/stream?streams=";
...
const restBase = (market: MarketKind) => (market === "perp" ? REST_FUT : REST_SPOT);
const wsBase = (market: MarketKind) => (market === "perp" ? WS_FUT : WS_SPOT);
...
export async function fetchKlines(symbol: string, interval: string, limit: number, market: MarketKind = "perp"): Promise<Candle[]> {
  const r = await fetch(`${restBase(market)}/klines?symbol=${symbol}&interval=${interval}&limit=${limit}`, { signal: withTimeout(9000) });
  ...
  return raw.map((k) => {
    ...
    return { t: Number(a[0]), o: Number(a[1]), h: Number(a[2]), l: Number(a[3]), c: Number(a[4]), v, delta: v > 0 ? tb * 2 - v : 0 };
  });
}

export async function fetchDepth(symbol: string, market: MarketKind = "perp"): Promise<BookData> {
  const r = await fetch(`${restBase(market)}/depth?symbol=${symbol}&limit=20`, { signal: withTimeout(9000) });
...
export async function fetchFundingOi(symbol: string): Promise<FundingOi | null> {
  const [pf, po] = await Promise.allSettled([
    fetch(`${REST_FUT}/premiumIndex?symbol=${symbol}`, { signal: withTimeout(9000) }),
    fetch(`${REST_FUT}/openInterest?symbol=${symbol}`, { signal: withTimeout(9000) }),
  ]);
  let funding = NaN, nextMs = NaN, markPrice = NaN;
  if (pf.status === "fulfilled" && pf.value.ok) {
    const j = (await pf.value.json()) as { lastFundingRate?: string; nextFundingTime?: number; markPrice?: string };
  ...
  if (po.status === "fulfilled" && po.value.ok) {
    ...
    const contracts = Number(j.openInterest);
    oi = Number.isFinite(markPrice) && Number.isFinite(contracts) ? contracts * markPrice : NaN;
  }
...
export function connectTickers(
  ...
  const open = () => {
    ...
    ws = new WebSocket(wsBase(market) + streams);
    ws.onopen = () => { retry = 0; everOpened = true; };
    ws.onmessage = (ev) => {
      try {
        const j = JSON.parse(ev.data as string) as { data?: { s: string; c: string; P: string; E?: number } };
        const d = j.data;
        if (d?.s && d.c) onTick({ symbol: d.s, price: Number(d.c), change24h: Number(d.P), evtTime: d.E });
      } catch {
        /* se ignora */
      }
    };
...
export function connectTrades(
  ...
  const open = () => {
    if (closed) return;
    ws = new WebSocket(wsBase(market) + `${symbol.toLowerCase()}@aggTrade`);
    ws.onopen = () => { retry = 0; };
    ws.onmessage = (ev) => {
      try {
