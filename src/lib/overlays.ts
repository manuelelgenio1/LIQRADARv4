...
export interface Sessions {
  pdh: number; pdl: number; pdo: number;
  sdh: number; sdl: number; sdo: number;
  hasPrev: boolean;
}
...
export function computeSessions(candles: { t: number; o: number; h: number; l: number }[], tfMin: number): Sessions {
  ...
  let firstCur = Infinity, firstPrev = Infinity;
  let hasPrev = false;
  for (const c of candles) {
    ...
    if (d === lastDay) {
      ...
      if (c.t < firstCur) { firstCur = c.t; sdo = c.o; }
    } else if (d === lastDay - 1) {
      hasPrev = true;
      pdh = Math.max(pdh, c.h);
      pdl = Math.min(pdl, c.l);
      if (c.t < firstPrev) { firstPrev = c.t; pdo = c.o; }
    }
  }
  return {
    pdh: hasPrev ? pdh : NaN,
...
export function computeVoids(candles: { t: number }[], heat: Float32Array, pMin: number, pMax: number, start: number): LiqVoid[] {
  ...
  if (maxAvg <= 0) return [];
  const COLD = maxAvg * 0.06;
  const HOT = maxAvg * 0.22;
...
export interface VolProfile {
  ...
  rows: Float64Array; rowH: number;
}

export function computeVolProfile(candles: { h: number; l: number; c: number; v: number }[], pMin: number, pMax: number, start: number): VolProfile | null {
  const span = pMax - pMin;
  ...
  return { poc, vah: pMin + (hi + 1) * rowH, val: pMin + lo * rowH, total, rows: vol, rowH };
}
...
export function computeLiqRegime(fundingPct: number, oiDeltaPct: number): LiqRegime {
  ...
  if (d < -1.2) return { label: "Despalancamiento", tone: "warn", note: "OI cayendo: liquidaciones en curso" };
  if (f > 0.03 && d > 0.4) return { label: "Longs aglomerados", tone: "short", note: "Funding caro + OI subiendo: riesgo de long-squeeze" };
  if (f < -0.03 && d > 0.4) return { label: "Shorts aglomerados", tone: "long", note: "Funding negativo + OI subiendo: riesgo de short-squeeze" };
  if (f > 0.03) return { label: "Sesgo largo caro", tone: "short", note: "Funding elevado: lado largo saturado" };
  if (f < -0.03) return { label: "Sesgo corto caro", tone: "long", note: "Funding negativo: lado corto saturado" };
  if (d > 0.8) return { label: "Apalancamiento en expansión", tone: "flat", note: "OI subiendo con funding neutro" };
  return { label: "Equilibrado", tone: "flat", note: "Funding neutro y OI estable" };
}
