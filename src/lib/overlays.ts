// ============================================================
// Overlays del heatmap: funciones puras de análisis.
// ============================================================
...
export function computeLiqRegime(fundingPct: number, oiDeltaPct: number): LiqRegime {
  ...
  if (d < -1.2) return { label: "Despalancamiento", tone: "warn", note: "OI cayendo: liquidaciones en curso, el movimiento pierde combustible" };
  if (f > 0.03 && d > 0.4) return { label: "Longs aglomerados", tone: "short", note: "Funding caro + OI subiendo: subida frágil, riesgo de long-squeeze" };
  if (f < -0.03 && d > 0.4) return { label: "Shorts aglomerados", tone: "long", note: "Funding negativo + OI subiendo: bajada frágil, riesgo de short-squeeze" };
  if (f > 0.03) return { label: "Sesgo largo caro", tone: "short", note: "Funding elevado: los largos pagan prima, el lado largo está saturado" };
  if (f < -0.03) return { label: "Sesgo corto caro", tone: "long", note: "Funding negativo: los cortos pagan prima, el lado corto está saturado" };
  if (d > 0.8) return { label: "Apalancamiento en expansión", tone: "flat", note: "OI subiendo con funding neutro: entra dinero nuevo" };
