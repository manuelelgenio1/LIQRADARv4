...
export function buildConsensus(
  ...
  const stUpLast = b.stUpConf.length ? b.stUpConf[b.stUpConf.length - 1] : true;
  votes.push({
    name: "Supertrend",
    note: `ATR ${cfg.atr} × ${cfg.stMult}`,
    weight: 1.25,
    dir: stUpLast ? "alcista" : "bajista",
    strength: 1,
  });
...
export function computeIndicators(candles: Candle[], cfg: IndicatorCfg, tfMinutes: number): IndicatorBundle {
  ...
  const consensus = buildConsensus(
    { emaFast, emaSlow, hist: m.hist, rsi, stUp: stR.up, stUpConf: stR.upConf, adx: adxR.adx, pdi: adxR.pdi, mdi: adxR.mdi, atr, closes, st: stR.line },
    cfg,
    tfMinutes
  );
  return {
