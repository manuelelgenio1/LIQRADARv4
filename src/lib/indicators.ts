...
// Calibrados por temporalidad (cada timeframe tiene sus periodos).
// ============================================================
...
export interface IndicatorCfg {
  ...
  trend: number;  // EMA de tendencia (largo plazo)
  ...
  adxThr: number; // umbral de régimen: ≥ = tendencia, < = rango
}
...
export function supertrendSeries(
  ...
  for (let i = 0; i < n; i++) {
    ...
    if (i === 0) trend = k.c > fu ? 1 : -1;
    else trend = trend === 1 ? (k.c < fl ? -1 : 1) : k.c > fu ? 1 : -1;
...
export function adxSeries(
  ...
  const atr = atrSeries(candles, period);
  let sp = 0, sm = 0;
  let dxSum = 0, dxCount = 0, adxPrev = 0;
  for (let i = 1; i < n; i++) {
...
export interface Consensus {
  dir: TrendDir;
  score: number;      // -1..1
  strength: number;   // convicción 0..1
  votes: Vote[];
}
...
export function buildConsensus(
  ...
  // umbral de separación de EMAs escalado al timeframe
  const thr = 0.0006 * Math.sqrt(tf / 5);

  // 1 · Cruce EMA
  const ef = last(b.emaFast), es = last(b.emaSlow);
  const sep = b.emaSlow.length && es !== 0 ? (ef - es) / es : 0;
  const emaDir: TrendDir = sep > thr ? "alcista" : sep < -thr ? "bajista" : "lateral";
  votes.push({
    name: "Cruce EMA",
  ...
  // 4 · Supertrend (giro confirmado)
  const st = b.stUpConf.length ? b.stUpConf[b.stUpConf.length - 1] : true;
  votes.push({
    name: "Supertrend",
...
// Recorta cada serie del bundle a las últimas `n` velas (semilla extendida).
...
// Ajuste del consenso por confluencia multi-timeframe.
