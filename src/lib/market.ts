...
export interface Candle {
  ...
  v: number;      // volumen (unidades)
  delta: number;  // delta comprador − vendedor
}
...
export interface SymbolMeta {
  ...
  liqScale: number; // escala de liquidaciones para el símbolo
  bookBase: number; // tamaño base del libro
}
...
export interface LiqCluster {
  ...
  side: "long" | "short";
  sizeUsd: number;
  strength: number; // 0..1 relativo al pico del calor
  leverage: string;
  exchange: string;
}
...
export interface BookLevel {
  price: number;
  size: number;
  total: number;   // profundidad acumulada
  exchange: string;
  isWall: boolean;
}
...
export interface LiquidationEvent {
  ...
  qtyUsd: number;
  exchange: string;
  isReal: boolean; // true SOLO si vino del websocket real (OKX)
}
...
export interface MarketState {
  ...
  tfMinutes: number;
  candles: Candle[];
  warm?: Candle[]; // semilla extendida (hasta 500 velas) para indicadores
  heat: Float32Array;
  heatMax: number;
  cvd: number[];
  pMin: number;
...
// cerca del precio → x100, lejos → x5. Garantiza todo el espectro
...
// ============================================================
// Derivación de clústeres a partir del calor (orden: distancia al precio)
// ============================================================
export function deriveClusters(
  ...
  const lastC = candles[candles.length - 1]?.c ?? meta.basePrice;
  const span = pMax - pMin || 1;

  // perfil medio de calor por bin sobre la ventana
  const profile = new Float64Array(HEAT_BINS);
  const start = Math.max(0, candles.length - 64);
  let cols = 0;
  for (let i = start; i < candles.length; i++) {
    cols++;
    for (let b = 0; b < HEAT_BINS; b++) profile[b] += heat[i * HEAT_BINS + b];
  }
  if (cols > 0) for (let b = 0; b < HEAT_BINS; b++) profile[b] /= cols;

  // picos = máximos locales genuinos con dedup por proximidad
  ...
  for (let b = 2; b < HEAT_BINS - 2; b++) {
    ...
    if (
      v > heatMax * 0.42 &&
      v >= profile[b - 1] && v >= profile[b + 1] &&
      v > profile[b - 2] && v > profile[b + 2]
    ) {
      if (used.some((u) => Math.abs(u - b) < 4)) continue;
      used.push(b);
      const price = pMin + ((b + 0.5) / HEAT_BINS) * span;
      const frac = Math.abs(price - lastC) / span;
      if (frac < 0.05) continue; // pegado al precio: no es un pool
      const side: "long" | "short" = price < lastC ? "long" : "short";
      clusters.push({
        id: `cl-${meta.symbol}-${b}`,
        price,
        side,
        sizeUsd: v * meta.liqScale * 1e6 * (1.6 + frac * 6),
        strength: Math.min(1, v / heatMax),
        leverage: levFromFrac(frac),
        exchange: EXCHANGES[b % 3],
      });
      if (clusters.length >= 9) break;
    }
  }

  // garantía: cada banda de apalancamiento presente en AMBOS lados
  const BANDS = [0.07, 0.16, 0.27, 0.39, 0.49];
  const ensure = (side: "long" | "short") => {
    for (let i = 0; i < BANDS.length; i++) {
      const has = clusters.some((c) => c.side === side && levFromFrac(Math.abs(c.price - lastC) / span) === levFromFrac(BANDS[i]));
      if (has) continue;
      const frac = BANDS[i] + ((hashStr(meta.symbol + side + i) % 100) / 100) * 0.06;
  ...
  // orden nativo: distancia al precio ascendente
  clusters.sort((a, b) => Math.abs(a.price - lastC) - Math.abs(b.price - lastC));
  return clusters.slice(0, 12);
}

// ============================================================
// Generación simulada (respaldo sin conexión)
// ============================================================
...
// Re-muestrea el calor cuando cambia el ancla de precios (pMin/pMax).
...
function jitterBook(meta: SymbolMeta, lastC: number, rand: () => number, dirUp: boolean): BookLevel[] {
  ...
  for (let i = 0; i < 15; i++) {
    const size = meta.bookBase * (0.6 + rand() * 2.4) * (1 + i * 0.12) + (rand() < 0.06 ? meta.bookBase * (4 + rand() * 6) : 0);
    total += size;
    levels.push({
...
// ---------- tick en vivo: aplica el precio real del websocket ----------
export function applyLiveTick(s: MarketState, price: number, tfMinutes: number): MarketState {
  ...
  // semilla extendida viva (para indicadores)
  let warm = s.warm;
  if (warm && warm.length) {
    const w = warm.slice();
    if (rolled) {
  ...
  // expansión SOLO si la vela supera el rango anclado (no por proximidad)
...
// ---------- merge de klines reales (cada 20 s en live) ----------
...
// ---------- tick del modelo (paneles vivos; sin deriva de datos en live) ----------
export function tickMarket(
  ...
  const pMin = s.pMin, pMax = s.pMax;
  let heat = s.heat;
  let heatMax = s.heatMax;
  let clusters = s.clusters;
  let cvd = s.cvd;
  let usedCandles = candles;

  if (withDrift) {
    // pequeña deriva del calor alrededor del precio vivo
    const span = pMax - pMin || 1;
    ...
    if (now - last.t >= stepMs) {
      ...
      usedCandles = [...candles.slice(1), nc];
      ...
      if (heatMax <= 0) heatMax = 1;
      cvd = s.cvd.slice(1);
      cvd.push(cvd[cvd.length - 1] + nc.delta);
    }
    clusters = deriveClusters(meta, usedCandles, heat, heatMax, pMin, pMax);
  }

  const events = withDrift ? maybeLiqEvent(s, rand, now) : s.events;

  // libro / spoofing / funding / OI: solo se mueven en simulado
...
// Delta real de trades (aggTrade) → CVD real
