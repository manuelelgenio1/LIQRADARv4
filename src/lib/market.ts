...
function seedHeat(candles: Candle[], pMin: number, pMax: number, rand: () => number): Float32Array {
  ...
  const span = pMax - pMin || 1;
  const binOf = (p: number) => Math.max(0, Math.min(HEAT_BINS - 1, Math.round(((p - pMin) / span) * (HEAT_BINS - 1))));
...
export function tickMarket(s: MarketState, opts?: { drift?: boolean; latencyMs?: number }): MarketState {
  ...
  if (withDrift) {
    ...
    clusters = deriveClusters(meta, candles, heat, heatMax, s.pMin, s.pMax);
  }

  const events = withDrift && rand() < 0.42
    ? [
        {
          id: `sim-${now}-${Math.floor(rand() * 1e6)}`,
          time: now,
          symbol: meta.symbol,
          side: (rand() > 0.5 ? "long" : "short") as "long" | "short",
          price: lastC * (1 + (rand() - 0.5) * 0.003),
          qtyUsd: meta.liqScale * 1e3 * (8 + rand() * rand() * 1600),
          exchange: EXCHANGES[Math.floor(rand() * 3)],
          isReal: false,
        },
        ...s.events,
  ...
  lat.push(opts?.latencyMs != null && Number.isFinite(opts.latencyMs) && opts.latencyMs >= 0 && opts.latencyMs < 3000 ? opts.latencyMs : 12 + rand() * 40);
