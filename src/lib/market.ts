// ============================================================
// Motor de mercado: estado, generación simulada (respaldo sin
// conexión) y funciones puras que inyectan datos 100% reales.
// ============================================================
...
export interface SymbolMeta {
  ...
  liqScale: number; // escala de liquidaciones
...
// límites compartidos: el heatmap dibuja los primeros CHART_CLUSTER_LIMIT y el
// radar los primeros RADAR_CLUSTER_LIMIT del mismo array ordenado por distancia
...
export function deriveClusters(
  ...
  const span = pMax - pMin || 1;
  // perfil vertical de calor (suma por bin en las últimas velas)
  ...
  for (const p of peaks) {
    ...
    if (frac < 0.05) continue; // demasiado pegado al precio
  ...
  // garantía: al menos 3 clústeres por lado, distribuidos por bandas
...
export function deriveState(meta: SymbolMeta, tfMinutes: number, candles: Candle[], seed: number): MarketState {
  ...
  return {
    meta, tfMinutes, candles, heat, heatMax, cvd, pMin, pMax, clusters, bids, asks,
...
export function marketFromKlines(meta: SymbolMeta, tfMinutes: number, klines: Candle[], seed: number): MarketState {
  ...
  const st = deriveState(meta, tfMinutes, klines.slice(-CANDLE_COUNT), seed);
...
export function applyLiveTick(s: MarketState, price: number, tfMinutes: number): MarketState {
  ...
  let heat = s.heat;
  if (pMin !== s.pMin || pMax !== s.pMax) heat = rebinHeat(s.heat, s.pMin, s.pMax, pMin, pMax);
...
export function tickMarket(s: MarketState, opts?: { drift?: boolean; latencyMs?: number }): MarketState {
  ...
  let cvd = s.cvd;
  let candlesOut = candles;

  if (withDrift) {
    heat = new Float32Array(s.heat);
    for (let i = 0; i < heat.length; i++) heat[i] *= 0.9988;
    const span = pMaxOf(s) - s.pMin || 1;
    const fb = ((lastC - s.pMin) / span) * (HEAT_BINS - 1);
    const ci = candles.length - 1;
    for (let db = -3; db <= 3; db++) {
      const bb = Math.round(fb) + db;
      if (bb >= 0 && bb < HEAT_BINS) heat[ci * HEAT_BINS + bb] += meta.bookBase * (0.5 + rand()) * Math.exp(-(db * db) / 2);
    }
    heatMax = 0;
    for (let i = 0; i < heat.length; i++) heatMax = Math.max(heatMax, heat[i]);
    ...
    clusters = deriveClusters(meta, candlesOut, heat, heatMax, s.pMin, s.pMax);
  }
  ...
  return {
    ...s, candles: candlesOut, heat, heatMax, cvd, clusters, events,
    bids, asks, imbalance, spoofing, funding, oi, oiDelta1h, longShortRatio,
...
function pMaxOf(s: MarketState): number {
  return s.pMax;
}
