    ...
    <section className="panel anim-reveal flex h-full flex-col" style={{ animationDelay: "0.24s" }}>
      ...
      <div className="scroll-slim max-h-[430px] min-h-0 flex-1 overflow-y-auto lg:max-h-none">
        ...
        {clusters.map((cl, i) => {
          ...
          return (
            <div key={cl.id}
              className={`group relative grid grid-cols-[44px_1fr_62px_70px_42px] items-center gap-2 border-b border-ink-700/25 px-4 py-[7px] transition-all duration-200 hover:bg-ink-750/60 ${isTarget ? "bg-ink-800/50" : ""}`}
              ...
              <div className="absolute inset-y-0 left-0 transition-all duration-700 group-hover:opacity-100"
                style={{
                  width: `${w * 0.35}%`, opacity: 0.3,
                  background: isLong ? "linear-gradient(90deg, rgba(45,224,192,0.28), transparent)" : "linear-gradient(90deg, rgba(255,93,126,0.28), transparent)",
                }} />
              {isTarget && (
              ...
              <div className="relative min-w-0 leading-tight">
                ...
                <div className="mt-0.5 flex items-center gap-2">
                  ...
                  <span className="h-1 w-14 overflow-hidden bg-ink-700/80">
                    <span className={`block h-full transition-all duration-700 ${isLong ? "bg-long-400/80" : "bg-short-400/80"}`}
