...
export default function ClusterList({ state }: Props) {
  ...
  // orden nativo de deriveClusters: distancia al precio ascendente
    ...
    <section className="panel anim-reveal flex h-full flex-col" style={{ animationDelay: "0.24s" }}>
      ...
      <div className="scroll-slim max-h-[430px] min-h-0 flex-1 overflow-y-auto lg:max-h-none">
        {clusters.map((cl, i) => {
          ...
          const isTarget = i === 0; // mismo objetivo que el Market Maker Path
          const isMagnet = cl.id === magnetId;
          const inChart = i < IN_CHART; // mismas líneas que dibuja el heatmap
      ...
      <footer className="border-t border-ink-700/50 bg-ink-900/50 px-4 py-2.5">
        <p className="font-mono text-[9px] leading-relaxed text-mist-600">
          ...
          el pool más cercano (lo caza primero el barrido); el <b className="text-flare-300">imán</b> es el de mayor nocional.
          *Tamaño estimado por el modelo — los niveles son reales, el volumen no.
        </p>
      </footer>
    </section>
  );
