...
interface Props { state: MarketState; market?: MarketKind; }
...
export default function ClusterList({ state, market = "perp" }: Props) {
  ...
  return (
    <section className="panel anim-reveal flex h-full flex-col" style={{ animationDelay: "0.32s" }}>
      <header className="flex items-center gap-3 border-b border-ink-700/50 px-4 py-3">
        <div className="leading-none">
          <h2 className="flex items-center gap-2 font-display text-sm font-bold uppercase tracking-[0.16em] text-mist-100">
            ...
            <span className="tick-num border border-ink-600 bg-ink-800 px-1.5 py-0.5 font-mono text-[9px] font-bold text-mist-300">{clusters.length}</span>
            ...
            }`} title={market === "perp" ? "Pools derivados del calor del PERPETUO de Binance Futuros" : "Pools derivados del calor del mercado SPOT"}>
              {market}
            </span>
          </h2>
      ...
      <div className="scroll-slim max-h-[430px] min-h-0 flex-1 overflow-y-auto lg:max-h-none">
        {clusters.length === 0 && (
          <div className="flex h-32 items-center justify-center font-mono text-[10px] uppercase tracking-widest text-mist-600">detectando clústeres de liquidez…</div>
        )}
        {clusters.map((cl, i) => {
          ...
          return (
            <div key={cl.id}
              className={`anim-feed-in group relative grid grid-cols-[44px_1fr_62px_70px_42px] items-center gap-2 border-b border-ink-700/25 px-4 py-[7px] transition-all duration-200 hover:bg-ink-750/60 ${isTarget ? "bg-ink-800/50" : ""}`}
              style={{ animationDelay: `${Math.min(i * 30, 300)}ms` }}>
              <div className="absolute inset-y-0 left-0 transition-all duration-700 group-hover:opacity-100"
                style={{ width: `${w * 0.35}%`, opacity: 0.3, background: isLong ? "linear-gradient(90deg, rgba(45,224,192,0.28), transparent)" : "linear-gradient(90deg, rgba(255,93,126,0.28), transparent)" }} />
              {isTarget && <span className="absolute left-0 top-1/2 h-6 w-[3px] -translate-y-1/2 bg-flare-400 shadow-[0_0_10px_rgba(255,178,36,0.7)]" />}
              <span className={`relative inline-flex w-fit items-center border px-1.5 py-0.5 font-mono text-[8.5px] font-bold uppercase tracking-wider ${
              ...
              <div className="relative min-w-0 leading-tight">
                <div className="flex items-center gap-1.5">
                  <span className="tick-num font-mono text-[11.5px] font-semibold text-mist-200">{fmtPrice(cl.price, state.meta.decimals)}</span>
                  {isTarget && (
                    <span className="flex items-center gap-1 border border-flare-400/50 bg-flare-400/10 px-1 py-px font-mono text-[7.5px] font-bold uppercase tracking-wider text-flare-300"
                  ...
                  {isMagnet && !isTarget && (
                    <span className="border border-flare-400/40 px-1 py-px font-mono text-[7.5px] font-bold uppercase tracking-wider text-flare-300/90"
                      title="Imán de liquidez: el clúster con mayor nocional estimado del rango">imán</span>
                  )}
                  {inChart && (
                    <span className="hidden border border-ink-600 px-1 py-px font-mono text-[7.5px] uppercase tracking-wider text-mist-500 sm:inline"
                      title="Este clúster tiene línea dibujada en el heatmap">gráfico</span>
                  )}
                </div>
                <div className="mt-0.5 flex items-center gap-2">
                  ...
                  <span className="h-1 w-14 overflow-hidden bg-ink-700/80">
                    <span className={`block h-full transition-all duration-700 ${isLong ? "bg-long-400/80" : "bg-short-400/80"}`} style={{ width: `${Math.round(cl.strength * 100)}%` }} />
                  </span>
                  <span className="tick-num font-mono text-[8px] text-mist-600">{Math.round(cl.strength * 100)}%</span>
                </div>
              </div>
              <span className={`tick-num relative text-right font-mono text-[10.5px] ${isLong ? "text-long-300" : "text-short-300"}`}>{fmtPct(dist, 2)}</span>
              <span className="tick-num relative text-right font-mono text-[11px] font-bold text-flare-300">{fmtUsd(cl.sizeUsd)}</span>
