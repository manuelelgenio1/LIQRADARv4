...
export function FundingOIPanel({ state, sentiment, live }: { state: MarketState; sentiment: LongShortRatio | null; live: boolean }) {
  // tres fuentes independientes: funding/OI vienen de fapi (live),
  // el ratio long/short de sentiment; cada una declara lo suyo
      ...
      <header className="flex items-center justify-between border-b border-ink-700/50 px-4 py-3">
        <h2 className="flex items-center gap-2 font-display text-sm font-bold uppercase tracking-[0.16em] text-mist-100">
          ...
          <span
            ...
            style={{
              background: live ? "#2de0c0" : "#5f7396",
              animation: live ? "liveBlink 1.6s ease-out infinite" : "none",
            }}
            title={live ? "Datos reales de Binance Futures" : "Datos del modelo (sin conexión)"}
          />
        </h2>
        <span className="font-mono text-[9px] uppercase tracking-widest text-mist-600" title="El funding y el OI solo existen en el mercado de perpetuos: siempre son de futuros, sin importar el toggle PERP/SPOT del gráfico">
          perpetuo · usdt
        </span>
      </header>
...
export function DataQualityPanel({
  ...
  const srcChip = (label: string, live: boolean, tip: string) => (
    <span
      className="flex items-center gap-1.5 border border-ink-700 bg-ink-850 px-2 py-1 font-mono text-[8.5px] uppercase tracking-wider text-mist-400"
      title={tip}
    >
      <span
        className={`h-1.5 w-1.5 rounded-full ${live ? "bg-long-400" : "bg-mist-600"}`}
  ...
  return (
    <section className="panel anim-reveal flex flex-1 flex-col" style={{ animationDelay: "0.36s" }}>
      ...
      <div className="grid grid-cols-3 divide-x divide-ink-700/50 border-t border-ink-700/50">
        ...
        <div className="px-3 py-2.5 text-center">
          <div className="tick-num font-display text-base font-bold text-mist-200">
            {source === "connecting" ? "…" : `${liveCount}/2`}
          </div>
      ...
      <div className="mt-auto flex flex-wrap gap-1.5 border-t border-ink-700/50 bg-ink-900/50 px-4 py-2.5">
        {srcChip(
          "Binance",
          source === "live",
          `Precio, velas, depth, funding y OI del mercado ${market === "perp" ? "de FUTUROS (perpetuo)" : "SPOT"} de Binance`
        )}
        {srcChip("OKX", liqSource === "okx", "Liquidaciones reales por websocket de OKX")}
        <span
          className={`ml-auto border px-2 py-1 font-mono text-[8.5px] font-bold uppercase tracking-widest ${
            market === "perp"
