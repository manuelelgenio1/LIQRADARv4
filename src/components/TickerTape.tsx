...
interface TapeItem { base: string; price: number; change: number; decimals: number; vol: number; }
    ...
    const id = window.setInterval(() => {
      ...
      setItems((prev) =>
        prev.map((it, i) => {
          ...
          return { ...it, price: it.price * (1 + drift), change: Math.max(-14, Math.min(14, it.change + drift * 35)) };
        })
      );
    }, 1600);
