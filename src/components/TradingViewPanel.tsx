...
function tvSymbol(base: string, market: MarketKind): string {
  return market === "perp" ? `BINANCE:${base}USDTPERP` : `BINANCE:${base}USDT`;
}

function tvUrl(base: string, tfKey: string, market: MarketKind): string {
  return `https://www.tradingview.com/chart/?symbol=${encodeURIComponent(tvSymbol(base, market))}&interval=${TV_TF[tfKey] ?? "5"}`;
}
    ...
    <div className="absolute inset-0 z-0 flex flex-col items-center justify-center gap-4 bg-ink-900/70">
      <div className="flex items-end gap-1.5">
        {[18, 30, 22, 40, 26, 34].map((h, i) => (
          <span key={i} className={`w-2.5 ${i % 2 ? "bg-short-400/70" : "bg-long-400/70"}`}
      ...
      <p className="font-mono text-[10px] uppercase tracking-[0.22em] text-mist-500">
        {mode === "loading" ? "conectando con TradingView…" : mode === "legacy" ? "cargando modo compatible…" : ""}
      </p>
    </div>
  );
}

export default function TradingViewPanel({ base, tfKey, market = "perp" }: Props) {
  const [open, setOpen] = useState<boolean>(() => !readFlag(OPEN_KEY) ? true : readFlag(OPEN_KEY));
  const [active, setActive] = useState<string[]>(() => {
    const s = readLS<string[]>(STUDY_KEY, []);
    return s.length ? s : ["EMA", "MACD", "RSI", "ADX", "ST"];
  });
  const [heightId, setHeightId] = useState<(typeof HEIGHTS)[number]["id"]>(() => {
    const h = readLS<string>(HEIGHT_KEY, "L");
    return h === "S" || h === "M" || h === "L" ? h : "L";
  });
  ...
  useEffect(() => {
    ...
    writeLS(STUDY_KEY, active);
    writeLS(HEIGHT_KEY, heightId);
  ...
  }, [open, fullscreen, base, tfKey, active, heightPx, market]);

  const legacySrc = `https://s.tradingview.com/widgetembed/?frameElementId=tv-liqradar&symbol=${encodeURIComponent(
    tvSymbol(base, market)
  )}&interval=${TV_TF[tfKey] ?? "5"}&hidesidetoolbar=0&symboledit=1&theme=dark&style=1&locale=es&studies=${encodeURIComponent(
    STUDIES.filter((st) => active.includes(st.id) && LEGACY_STUDIES[st.id]).map((st) => LEGACY_STUDIES[st.id]).join(",")
  )}`;

  const chartArea = (isFs: boolean) => (
    <>
      {mode !== "widget" && <LoadingMark mode={mode} />}
      <div ref={isFs ? fsHolderRef : holderRef} className="tradingview-widget-container relative z-10 h-full w-full" />
      {mode === "legacy" && (
        <iframe title="TradingView (modo compatible)" src={legacySrc}
          className="absolute inset-0 z-20 h-full w-full border-0" allow="fullscreen" />
      )}
    </>
  );
      ...
      return (
        <button key={st.id}
          ...
          className={`shrink-0 font-mono text-[9px] font-semibold uppercase tracking-wider transition-all duration-150 ${
            ...
            ? compact ? "border-long-500/50 bg-long-900/40 text-long-300" : "bg-long-500/15 text-long-300 shadow-[inset_0_-2px_0_rgba(45,224,192,0.55)]"
            : compact ? "border-ink-700 bg-ink-850 text-mist-600 hover:text-mist-400" : "text-mist-600 hover:bg-ink-750 hover:text-mist-400"
          }`}
  ...
  const modeBadge =
    mode === "widget" ? { t: "widget oficial", c: "border-long-500/50 bg-long-900/40 text-long-300" }
      : mode === "legacy" ? { t: "modo compatible", c: "border-flare-400/50 bg-flare-400/10 text-flare-300" }
    ...
    <>
      <section className="panel anim-reveal" style={{ animationDelay: "0.66s" }}>
          ...
          <div className="flex items-center gap-2.5 leading-none">
            ...
            <div>
              ...
              <p className="mt-1 flex items-center gap-1.5 font-mono text-[10px] uppercase tracking-widest text-mist-500">
                ...
                <b className="text-mist-300">{base}USDT{market === "perp" ? "PERP" : ""}</b> · {tfKey} · {market === "perp" ? "perpetuo" : "spot"} · sincronizado con el radar
              </p>
            </div>
          </div>
          ...
          <div className="ml-auto flex flex-wrap items-center gap-2">
            <div className="flex flex-wrap items-center border border-ink-700 bg-ink-850/80">
              <span className="border-r border-ink-700 px-2 py-1.5 font-mono text-[8px] font-bold uppercase tracking-widest text-mist-600">
                ind <b className="tick-num ml-1 text-long-300">{active.length}/{STUDIES.length}</b>
              </span>
              {studyChips(false)}
            </div>
            ...
            <div className="flex items-center border border-ink-700 bg-ink-850/80" title="Altura del gráfico">
              {HEIGHTS.map((h) => (
                <button key={h.id} onClick={() => setHeightId(h.id)}
                  className={`px-2 py-1.5 font-mono text-[9px] font-bold transition-colors ${
                    h.id === heightId ? "bg-mist-200/15 text-mist-100" : "text-mist-600 hover:bg-ink-750 hover:text-mist-400"
                  }`}
                  title={h.label}>
                  {h.id}
                </button>
        ...
        <footer className="flex flex-wrap items-center justify-between gap-2 border-t border-ink-700/50 bg-ink-900/50 px-4 py-2 font-mono text-[8.5px] uppercase tracking-widest text-mist-600">
          <span>
            velas japonesas · mismo símbolo ({market === "perp" ? "perpetuo" : "spot"}) y temporalidad del radar
            {mode === "legacy" && " · (embed alternativo porque el widget oficial no cargó)"}
          </span>
          <a href={tvUrl(base, tfKey, market)} target="_blank" rel="noreferrer"
        ...
        <div className="fixed inset-0 z-50 flex flex-col bg-ink-950">
          <div className="flex h-12 shrink-0 items-center gap-3 border-b border-ink-700/60 bg-ink-900/90 px-4">
            <span className="flex items-center gap-2 font-display text-xs font-bold uppercase tracking-[0.18em] text-mist-100">
              ...
              Análisis clásico · <span className="text-long-400">{base}USDT{market === "perp" ? "PERP" : ""}</span> · {tfKey}
            </span>
