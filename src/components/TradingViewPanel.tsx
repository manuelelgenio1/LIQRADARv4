...
// símbolo correcto según el mercado: el perpetuo en TradingView es BTCUSDTPERP
function tvSymbol(base: string, market: "perp" | "spot"): string {
  return market === "perp" ? `BINANCE:${base}USDTPERP` : `BINANCE:${base}USDT`;
}

function tvUrl(base: string, tfKey: string, market: "perp" | "spot"): string {
...
export default function TradingViewPanel({ base, tfKey, market = "perp" }: Props) {
  ...
  // Inyección del widget con altura EXPLÍCITA medida del contenedor.
  useEffect(() => {
          ...
          <div className="flex items-center gap-2.5 leading-none">
            ...
            <div>
              ...
              <p className="mt-1 flex items-center gap-1.5 font-mono text-[10px] uppercase tracking-widest text-mist-500">
                ...
                <b className="text-mist-300">{base}USDT{market === "perp" ? "PERP" : ""}</b> · {tfKey} ·{" "}
                {market === "perp" ? "perpetuo" : "spot"} · sincronizado con el radar
              </p>
            </div>
          </div>
