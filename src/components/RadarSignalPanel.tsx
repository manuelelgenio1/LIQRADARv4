...
interface SignalPart { label: string; value: number; weight: number; note: string; color: string; }
...
export default function RadarSignalPanel({ state, ind, confluence, market = "perp" }: Props) {
  const parts = useMemo<SignalPart[]>(() => {
    ...
    out.push({
      label: "Consenso 5-ind", value: consSign * cons.strength, weight: 1.2,
    ...
    if (confluence && confluence.length) {
      ...
      out.push({
        label: "Confluencia MTF", value: align, weight: 1.0,
    ...
    out.push({ label: "Funding + OI", value: regVal, weight: 0.8, note: regime.label, color: regVal > 0 ? "#2de0c0" : regVal < 0 ? "#ff5d7e" : "#5f7396" });
    const cvd = state.cvd;
    if (cvd.length >= 2) {
      ...
      const cvdVal = clamp((last - prev) / (mag * 1.5), -1, 1);
      out.push({
        label: "CVD (order flow)", value: cvdVal, weight: 0.9,
        note: `${cvdVal > 0.1 ? "flujo comprador" : cvdVal < -0.1 ? "flujo vendedor" : "flujo neutro"} reciente · CVD ${fmtCompact(last)}`,
    ...
    if (clusters.length) {
      ...
      for (const c of clusters) (c.side === "long" ? (longLiq += c.sizeUsd) : (shortLiq += c.sizeUsd));
      const asym = shortLiq + longLiq > 0 ? (shortLiq - longLiq) / (shortLiq + longLiq) : 0;
      out.push({
        label: "Asimetría pools", value: clamp(asym, -1, 1), weight: 0.7,
  ...
  const { bias, conviction } = useMemo(() => {
    ...
    for (const p of parts) { num += p.value * p.weight; den += p.weight; conv += Math.abs(p.value) * p.weight; }
    return { bias: den ? num / den : 0, conviction: den ? conv / den : 0 };
  }, [parts]);

  const dir: TrendDir = bias > 0.15 ? "alcista" : bias < -0.15 ? "bajista" : "lateral";
  const dirMeta = dir === "alcista" ? { label: "Sesgo alcista", text: "text-long-300" }
    : dir === "bajista" ? { label: "Sesgo bajista", text: "text-short-300" }
    : { label: "Sin sesgo claro", text: "text-mist-300" };
      ...
      <div className="flex flex-col gap-4 px-4 py-3.5 lg:flex-row lg:items-center lg:gap-8">
        ...
        <div className="min-w-0 flex-1">
          ...
          <div className="relative h-3 overflow-hidden border border-ink-700 bg-ink-800">
            ...
            <div className="absolute inset-y-0 w-[3px] -translate-x-1/2 transition-all duration-700 ease-out" style={{ left: `${needleLeft}%`, background: needleColor, boxShadow: `0 0 ${4 + conviction * 8}px ${needleColor}` }} />
          </div>
        ...
        <div className="grid shrink-0 grid-cols-2 gap-x-5 gap-y-1.5 sm:grid-cols-3 lg:w-[330px] lg:grid-cols-1">
          {parts.map((p, i) => (
            <div key={p.label} className="anim-feed-in group flex items-center gap-2 rounded-sm px-1 transition-colors hover:bg-ink-800/40" style={{ animationDelay: `${i * 60}ms` }} title={p.note}>
