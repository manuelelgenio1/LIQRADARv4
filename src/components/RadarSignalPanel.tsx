...
export default function RadarSignalPanel({ state, ind, confluence, market = "perp" }: Props) {
  const parts = useMemo<SignalPart[]>(() => {
    ...
    if (cvd.length >= 2) {
      ...
      out.push({
        ...
        value: cvdVal,
        weight: 0.9,
        note: `${cvdVal > 0.1 ? "flujo comprador" : cvdVal < -0.1 ? "flujo vendedor" : "flujo neutro"} reciente · CVD acumulado ${fmtCompact(last)}`,
  ...
  const dirMeta =
    dir === "alcista"
      ? { label: "Sesgo alcista", text: "text-long-300" }
      : dir === "bajista"
        ? { label: "Sesgo bajista", text: "text-short-300" }
      ...
      <div className="flex flex-col gap-4 px-4 py-3.5 lg:flex-row lg:items-center lg:gap-8">
        <div className="flex items-center gap-3 lg:w-[250px] lg:shrink-0">
          <div
            ...
            style={{
              ...
              boxShadow: `0 0 ${8 + conviction * 18}px ${
                dir === "alcista" ? "rgba(45,224,192,0.25)" : dir === "bajista" ? "rgba(255,93,126,0.25)" : "transparent"
              }`,
            }}
          >
            {dir === "lateral" ? (
          ...
          <div className="leading-none">
            ...
            <div key={dir} className={`anim-feed-in mt-1 font-display text-lg font-bold uppercase tracking-wide ${dirMeta.text}`}>
              {dirMeta.label}
            </div>
          </div>
        </div>
          ...
          <div className="relative h-3 overflow-hidden border border-ink-700 bg-ink-800">
            ...
            <div
              ...
              style={{
                ...
                boxShadow: `0 0 ${4 + conviction * 8}px ${needleColor}`,
              }}
            />
          </div>
        ...
        <div className="grid shrink-0 grid-cols-2 gap-x-5 gap-y-1.5 sm:grid-cols-3 lg:w-[330px] lg:grid-cols-1">
          {parts.map((p, i) => (
            <div
              key={p.label}
              className="anim-feed-in group flex items-center gap-2 rounded-sm transition-colors hover:bg-ink-800/40"
              style={{ animationDelay: `${i * 60}ms` }}
              title={p.note}
            >
              <span className="w-[108px] shrink-0 truncate font-mono text-[8px] uppercase tracking-wider text-mist-500 transition-colors group-hover:text-mist-300">
                {p.label}
              </span>
