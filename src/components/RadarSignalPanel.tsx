...
interface SignalPart {
  ...
  weight: number;
  note: string;
  color: string;   // color de la barra de contribución
}

const clamp = (v: number, lo: number, hi: number) => Math.max(lo, Math.min(hi, v));

export default function RadarSignalPanel({ state, ind, confluence }: Props) {
  const parts = useMemo<SignalPart[]>(() => {
    const out: SignalPart[] = [];

    // 1 · Consenso de 5 indicadores (ya ponderado por confluencia en el heatmap)
    ...
    let regVal = 0;
    if (regime.tone === "long") regVal = 0.6;        // shorts aglomerados → riesgo de squeeze alcista
    else if (regime.tone === "short") regVal = -0.6; // longs aglomerados → riesgo de squeeze bajista
  ...
  const biasPct = clamp(bias, -1, 1) * 100; // -100 … +100
  const needleLeft = 50 + biasPct / 2;       // posición 0-100% en la barra
      ...
      <div className="flex flex-col gap-4 px-4 py-3.5 lg:flex-row lg:items-center lg:gap-8">
        ...
        <div className="flex items-center gap-3 lg:w-[230px] lg:shrink-0">
          ...
          <div className="leading-none">
            <div className="font-mono text-[8.5px] uppercase tracking-[0.2em] text-mist-600">Señal del radar</div>
        ...
        <div className="min-w-0 flex-1">
          ...
          <div className="relative h-3 overflow-hidden border border-ink-700 bg-ink-800">
            {/* gradiente bajista → neutro → alcista */}
            <div
