...
export default function RadarScope({ state }: { state: MarketState }) {
  ...
  const blips = useMemo(() => {
    const picked = state.clusters.slice(0, RADAR_CLUSTER_LIMIT);
    const pickedIds = new Set(picked.map((c) => c.id));
    for (const lev of ["x100", "x50", "x20", "x10", "x5"]) {
      if (picked.some((c) => c.leverage === lev)) continue;
      const fill = state.clusters.find((c) => c.leverage === lev && !pickedIds.has(c.id));
      if (fill) {
        picked.push(fill);
        pickedIds.add(fill.id);
      }
    }
    return picked.map((cl) => {
      ...
      const angle = cl.side === "short" ? (-150 + jitter * 120) * (Math.PI / 180) : (30 + jitter * 120) * (Math.PI / 180);
      return { cl, x: C + Math.cos(angle) * r, y: C + Math.sin(angle) * r, size: Math.min(8.5, 3 + Math.sqrt(cl.sizeUsd / 1e6) * 1.5) };
    });
    ...
    <section className="panel panel-corner anim-reveal flex h-full flex-col" style={{ animationDelay: "0.12s" }}>
      <header className="flex items-center gap-3 border-b border-ink-700/50 px-4 py-3">
        <div className="leading-none">
          ...
          <p className="mt-1.5 font-mono text-[10px] uppercase tracking-widest text-mist-500">clústeres por apalancamiento · x100 → x5</p>
        </div>
      ...
      <div className="flex min-h-0 flex-1 items-center justify-center px-3 py-4">
        <div className="relative w-full max-w-[420px]">
          <svg viewBox={`0 0 ${S} ${S}`} className="w-full" onMouseLeave={() => setHovered(null)}>
            ...
            {BANDS.map((b) => (
              <g key={b.lev}>
                ...
                <text x={C + 5} y={C - radiusFor(b.frac) + 11} fill="#48597a" fontSize="8.5" fontFamily="IBM Plex Mono, monospace">{b.lev}</text>
              </g>
            ))}
            ...
            {Array.from({ length: 24 }).map((_, i) => {
              ...
              return <line key={i} x1={C + Math.cos(a) * 146} y1={C + Math.sin(a) * 146} x2={C + Math.cos(a) * 152} y2={C + Math.sin(a) * 152} stroke="#253650" strokeWidth="1.5" />;
            })}
            <text x={C} y={24} fill="#5f7396" fontSize="8.5" textAnchor="middle" fontFamily="IBM Plex Mono, monospace" letterSpacing="2">SHORTS ↑</text>
            <text x={C} y={S - 16} fill="#5f7396" fontSize="8.5" textAnchor="middle" fontFamily="IBM Plex Mono, monospace" letterSpacing="2">LONGS ↓</text>
            ...
            {blips.map((b) => {
              ...
              return (
                <g key={b.cl.id} onMouseEnter={() => setHovered({ cl: b.cl, x: b.x, y: b.y })} style={{ cursor: "pointer" }}>
                  <circle cx={b.x} cy={b.y} r={b.size + 10} fill="transparent" />
                  <circle cx={b.x} cy={b.y} r={b.size + 3} fill="none" stroke={col} strokeWidth="1"
                    style={{ transformOrigin: `${b.x}px ${b.y}px`, animation: "blipPulse 2.4s ease-in-out infinite" }} />
                  <circle cx={b.x} cy={b.y} r={b.size} fill={col} opacity={isHov ? 1 : 0.8} stroke="#070c16" strokeWidth="1"
                    style={{ transform: isHov ? "scale(1.35)" : "scale(1)", transformOrigin: `${b.x}px ${b.y}px`, transition: "transform 0.2s ease, opacity 0.2s ease", filter: isHov ? `drop-shadow(0 0 5px ${col})` : "none" }} />
          ...
          {hovered && (
            <div className="pointer-events-none absolute z-20 w-48 -translate-x-1/2 border border-ink-600 bg-ink-900/95 px-3 py-2 text-center font-mono text-[10px] shadow-xl"
              style={{ left: `${(hovered.x / S) * 100}%`, top: `calc(${(hovered.y / S) * 100}% + 26px)` }}>
              ...
              <div className="tick-num mt-0.5 text-mist-200">{fmtPrice(hovered.cl.price, state.meta.decimals)} · {fmtUsd(hovered.cl.sizeUsd)}</div>
          ...
          {blips.length === 0 && (
            <div className="absolute inset-0 flex items-center justify-center font-mono text-[10px] uppercase tracking-widest text-mist-600">escaneando liquidez…</div>
          )}
        </div>
      </div>

      <footer className="grid grid-cols-3 divide-x divide-ink-700/50 border-t border-ink-700/50 bg-ink-900/50">
        <div className="px-3 py-2.5 text-center transition-colors hover:bg-ink-800/60">
          <div className="font-mono text-[9px] uppercase tracking-widest text-mist-600">Longs ↓</div>
          <div className="tick-num mt-0.5 font-display text-sm font-bold text-long-300">{longBelow.length} · {fmtUsd(longBelow.reduce((s, c) => s + c.sizeUsd, 0))}</div>
        </div>
        <div className="px-3 py-2.5 text-center transition-colors hover:bg-ink-800/60">
        ...
        <div className="px-3 py-2.5 text-center transition-colors hover:bg-ink-800/60">
          ...
          <div className="tick-num mt-0.5 font-display text-sm font-bold text-short-300">{shortAbove.length} · {fmtUsd(shortAbove.reduce((s, c) => s + c.sizeUsd, 0))}</div>
        </div>
      </footer>
    </section>
  );
