...
import type { SymbolMeta, MarketState } from "../lib/market";
import type { Source } from "../hooks/useMarketEngine";
import type { MarketKind, TickerInfo } from "../lib/live";
import { fmtPrice, fmtPct, fmtClock } from "../lib/format";
...
export default function TopBar({
  ...
  const change = lp && !paused ? lp.change24h : 0;
  const up = change >= 0;
      ...
      <div className="mx-auto flex h-16 max-w-[1600px] items-center gap-4 px-4 lg:px-6">
        ...
        <div className="ml-auto flex items-center gap-3">
          <div
            ...
            title="Fuente de los datos: PERP usa el precio del perpetuo de Binance Futuros; SPOT usa el mercado al contado. Difieren por el basis."
          >
          ...
          <div className="text-right leading-none">
            ...
            <div className="mt-1 font-mono text-[9px] uppercase tracking-[0.2em] text-mist-600">
              {source === "live" ? `${market === "perp" ? "futuros" : "spot"} · usdt · ws en vivo` : "perpetuo · usdt"}
            </div>
          </div>
          ...
          <button
            ...
            className={`hidden items-center gap-1.5 border px-2.5 py-2 font-mono text-[10px] font-semibold uppercase tracking-widest transition-all sm:flex ${
              alertsOn
                ...
                : "border-ink-700 bg-ink-850/80 text-mist-500 hover:text-mist-300"
            }`}
            title={alertsOn ? "Alertas de liquidación activas (sonido)" : "Activar alertas de liquidación"}
          >
            <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"
              style={alertsOn ? { animation: "bellSwing 2.5s ease-in-out infinite", transformOrigin: "top center" } : undefined}>
