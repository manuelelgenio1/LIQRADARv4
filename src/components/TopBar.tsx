...
import type { MarketKind, TickerInfo } from "../lib/live";
...
export default function TopBar({
  meta, state, symbols, symbol, setSymbol, paused, setPaused, source, livePrices, alertsOn, toggleAlerts, market, onMarket,
}: Props) {
  ...
  const change = lp && !paused ? lp.change24h : ((price - state.candles[0].o) / state.candles[0].o) * 100;
  const up = change >= 0;
    ...
    <header className="sticky top-0 z-40 border-b border-ink-700/60 bg-ink-900/85 backdrop-blur-md">
      <div className="mx-auto flex h-16 max-w-[1600px] items-center gap-4 px-4 lg:px-6">
        ...
        <nav className="scroll-slim hidden items-center gap-1 overflow-x-auto md:flex">
          {symbols.map((s) => {
            ...
            return (
              ...
              >
                {s.base}
                {sp && <span className={`ml-1.5 text-[9px] ${sp.change24h >= 0 ? "text-long-400" : "text-short-400"}`}>{sp.change24h >= 0 ? "▲" : "▼"}</span>}
              </button>
            );
          })}
        </nav>
        ...
        <div className="ml-auto flex items-center gap-3">
          <div className="hidden items-stretch border border-ink-700 bg-ink-850/80 sm:flex" title="Fuente de datos: PERP = perpetuo de Binance Futuros, SPOT = mercado al contado">
          ...
          <div className="text-right leading-none">
            <div className="flex items-baseline justify-end gap-2">
              ...
              <span className={`tick-num border px-1.5 py-0.5 font-mono text-[10px] font-semibold ${up ? "border-long-500/40 bg-long-900/50 text-long-300" : "border-short-500/40 bg-short-900/50 text-short-300"}`}>
                {fmtPct(change)}
              </span>
            </div>
            <div className="mt-1 font-mono text-[9px] uppercase tracking-[0.2em] text-mist-600">
              {source === "live" ? `${market === "perp" ? "futuros" : "spot"} · usdt · ws` : "perpetuo · usdt"}
            </div>
          </div>
          ...
          <button
            ...
            className={`hidden items-center gap-1.5 border px-2.5 py-2 font-mono text-[10px] font-semibold uppercase tracking-widest transition-all sm:flex ${
              alertsOn ? "border-long-500/40 bg-long-900/30 text-long-300 hover:bg-long-900/60" : "border-ink-700 bg-ink-850/80 text-mist-500 hover:text-mist-300"
            }`}
            title={alertsOn ? "Alertas activas (sonido)" : "Activar alertas"}
          >
            <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" style={alertsOn ? { animation: "bellSwing 2.5s ease-in-out infinite", transformOrigin: "top center" } : undefined}>
              <path d="M18 8a6 6 0 0 0-12 0c0 7-3 9-3 9h18s-3-2-3-9" strokeLinecap="round" strokeLinejoin="round" />
              <path d="M13.7 21a2 2 0 0 1-3.4 0" strokeLinecap="round" />
            </svg>
            {alertsOn ? "Alertas" : "Mudo"}
          </button>

          <button
            onClick={() => setPaused(!paused)}
            className={`flex items-center gap-2 border px-3 py-2 font-mono text-[10px] font-semibold uppercase tracking-[0.18em] transition-all ${
              paused ? "border-flare-400/50 bg-flare-400/10 text-flare-300 hover:bg-flare-400/20" : "border-long-500/40 bg-long-900/30 text-long-300 hover:bg-long-900/60"
            }`}
            title={paused ? "Reanudar feed" : "Pausar feed"}
          >
            <span className={`h-2 w-2 rounded-full ${paused ? "bg-flare-400" : "bg-long-400"}`} style={{ animation: paused ? "none" : "liveBlink 1.6s ease-out infinite" }} />
      ...
      <nav className="scroll-slim flex items-center gap-1 overflow-x-auto border-t border-ink-700/50 px-3 py-2 md:hidden">
        {symbols.map((s) => (
          <button
            key={s.symbol}
            onClick={() => setSymbol(s.symbol)}
            className={`shrink-0 border px-2.5 py-1 font-mono text-[11px] font-semibold ${s.symbol === symbol ? "border-long-500/60 bg-long-900/40 text-long-300" : "border-transparent text-mist-500"}`}
          >
            {s.base}
          </button>
        ))}
