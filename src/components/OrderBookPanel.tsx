...
function Row({ lv, max, side, decimals, source }: { lv: BookLevel; max: number; side: "bid" | "ask"; decimals: number; source: string }) {
  ...
  return (
    <div className="group relative flex items-center gap-2 px-3 py-[3px] font-mono text-[10.5px] transition-colors hover:bg-ink-750/60">
      {/* barra de profundidad acumulada (animada: el libro se siente vivo) */}
      <div
      ...
      {lv.isWall && (
        <span
          className="absolute inset-y-[3px] left-0 w-[3px]"
          style={{ background: border, boxShadow: `0 0 8px ${border}` }}
      ...
      <span
        className={`relative w-[52px] shrink-0 text-right text-[8.5px] uppercase tracking-wider ${
          lv.isWall ? "font-bold text-flare-300" : "text-mist-600"
        }`}
      >
...
export default function OrderBookPanel({ state, live, market = "perp" }: Props) {
  ...
  // guard: mientras no haya libro (conectando) no se puede renderizar la tabla
  if (!bids.length || !asks.length) {
    return (
      <section className="panel anim-reveal flex h-full flex-col" style={{ animationDelay: "0.18s" }}>
        ...
        <div className="flex flex-1 items-center justify-center font-mono text-[10px] uppercase tracking-widest text-mist-600">
          recibiendo profundidad…
        </div>
      </section>
    );
  }
  ...
  // etiqueta de fuente por fila: en vivo todo viene del depth spot de Binance;
  // en simulado se muestra el exchange agregado de cada nivel
  const rowSource = live ? "spot" : "";
      ...
      <header className="flex items-center gap-3 border-b border-ink-700/50 px-4 py-3">
        <div className="leading-none">
          <h2 className="flex items-center gap-2 font-display text-sm font-bold uppercase tracking-[0.16em] text-mist-100">
            ...
            {live && (
              <span
                className={`h-1.5 w-1.5 rounded-full bg-long-400 transition-opacity duration-300 ${flash ? "opacity-100" : "opacity-30"}`}
                style={{ boxShadow: flash ? "0 0 8px rgba(45,224,192,0.8)" : "none" }}
                title="Parpadea con cada snapshot nuevo del depth"
              />
            )}
          </h2>
        ...
        <span
          className={`ml-auto border px-2 py-1 font-mono text-[9px] font-bold uppercase tracking-widest ${spoofLabel.c}`}
      ...
      <div className="flex min-h-0 flex-1 flex-col justify-center overflow-hidden">
        {asks.slice(0, 8).reverse().map((lv, i) => (
          <Row
            ...
            source={rowSource || lv.exchange}
          />
        ))}

        {/* mid + spread + tamaño en el best bid/ask (parpadea con cada snapshot) */}
        <div
          className={`flex items-center gap-3 border-y border-ink-700/60 bg-ink-900/80 px-3 py-2 transition-shadow duration-500 ${
            flash ? "shadow-[inset_0_0_18px_rgba(45,224,192,0.12)]" : ""
          }`}
        >
