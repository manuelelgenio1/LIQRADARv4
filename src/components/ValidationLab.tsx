...
import type { PoolRecord, PoolStats, BacktestResult } from "../lib/validation";
import { runBacktest } from "../lib/validation";
...
function StatusBadge({ r, now }: { r: PoolRecord; now: number }) {
  ...
  if (r.status === "expirado") {
    return <span className="border border-ink-700 bg-ink-850 px-1.5 py-0.5 font-mono text-[8px] font-bold uppercase tracking-wider text-mist-600">expirado</span>;
  }
  if (!r.outcome) {
    return <span className="border border-flare-400/50 bg-flare-400/10 px-1.5 py-0.5 font-mono text-[8px] font-bold uppercase tracking-wider text-flare-300">barrido · midiendo…</span>;
  }
  const map = {
  ...
  return <span className={`border px-1.5 py-0.5 font-mono text-[8px] font-bold uppercase tracking-wider ${m.c}`}>{m.t}</span>;
}

export default function ValidationLab({ log, stats, symbol, decimals, lastSync, market = "perp", candles }: Props) {
  ...
  // invalidar backtest al cambiar símbolo/mercado
  useEffect(() => {
    setBt(null);
  }, [symbol, market]);

  const symRecords = log.filter((r) => r.symbol === symbol && r.market === market);
  ...
  const syncAgo = lastSync > 0 ? fmtAgo(lastSync, now) : null;
  const delta = Number.isFinite(stats.hitRate) && Number.isFinite(stats.controlHitRate) ? stats.hitRate - stats.controlHitRate : NaN;

  const verdictMeta: Record<BacktestResult["verdict"], { c: string }> = {
    "SEÑAL REAL": { c: "border-long-500/50 bg-long-900/40 text-long-300" },
    "RUIDO": { c: "border-short-500/50 bg-short-900/40 text-short-300" },
    "INDETERMINADO": { c: "border-flare-400/50 bg-flare-400/10 text-flare-300" },
    "DATOS INSUFICIENTES": { c: "border-ink-600 bg-ink-800 text-mist-400" },
  };
    ...
    <section className="panel panel-corner anim-reveal" style={{ animationDelay: "0.6s" }}>
      <header className="flex flex-wrap items-center gap-x-3 gap-y-2 border-b border-ink-700/50 px-4 py-3">
        <div className="leading-none">
          ...
          <p className="mt-1 flex items-center gap-1.5 font-mono text-[10px] uppercase tracking-widest text-mist-500">
            ...
            <span className={`border px-1 py-px font-mono text-[7.5px] font-bold uppercase tracking-widest ${
        ...
        <div className="ml-auto flex items-center gap-2">
          ...
          <button onClick={runBt} disabled={btRunning || candles.length < 100}
            className={`flex items-center gap-1.5 border px-2.5 py-1.5 font-mono text-[10px] font-semibold uppercase tracking-widest transition-all ${
              btRunning || candles.length < 100
                ? "cursor-default border-ink-700 text-mist-600"
                : "border-long-500/40 bg-long-900/30 text-long-300 hover:bg-long-900/60"
            }`}
            title={candles.length < 100 ? "Se necesitan más velas para el backtest" : "Validar los pools contra el historial"}>
            {btRunning ? (
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12">
        ...
        <div className="grid grid-cols-2 gap-px border-b border-ink-700/50 bg-ink-700/30 sm:grid-cols-4 lg:col-span-4 lg:border-b-0 lg:border-r">
          <div className="bg-ink-900/60 px-4 py-3">
            ...
            <div className="tick-num mt-1 font-display text-2xl font-bold text-mist-100">{Number.isFinite(stats.hitRate) ? `${(stats.hitRate * 100).toFixed(0)}%` : "—"}</div>
          ...
          <div className="bg-ink-900/60 px-4 py-3">
            ...
            <div className={`tick-num mt-1 font-display text-2xl font-bold ${Number.isFinite(stats.reversalRate) && stats.reversalRate >= 0.5 ? "text-long-300" : "text-mist-100"}`}>
              {Number.isFinite(stats.reversalRate) ? `${(stats.reversalRate * 100).toFixed(0)}%` : "—"}
            </div>
            <div className="mt-1 font-mono text-[9px] text-mist-600">{stats.reversals}↑ / {stats.continuations}↓ resueltos</div>
          </div>
          <div className="bg-ink-900/60 px-4 py-3">
          ...
          <div className="bg-ink-900/60 px-4 py-3">
            ...
            <div className="mt-1 font-mono text-[9px] text-mist-600">{stats.swept} barridos · {stats.expired} expirados</div>
          </div>
          ...
          <div className="col-span-2 bg-ink-900/60 px-4 py-3 sm:col-span-4">
            ...
            <div className="flex h-2 overflow-hidden bg-ink-800">
              <div className="h-full bg-mist-500/70 transition-all duration-700" style={{ width: `${(stats.pending / Math.max(1, stats.total)) * 100}%` }} />
              <div className="h-full bg-flare-400/85 transition-all duration-700" style={{ width: `${(stats.swept / Math.max(1, stats.total)) * 100}%` }} />
              <div className="h-full bg-ink-600 transition-all duration-700" style={{ width: `${(stats.expired / Math.max(1, stats.total)) * 100}%` }} />
            </div>
            ...
            {syncAgo && (
              <div className="mt-1.5 font-mono text-[8.5px] uppercase tracking-widest text-mist-600">
                última sincronización <b className="text-mist-300">hace {syncAgo}</b>
              </div>
            )}
          </div>
        </div>
        ...
        <div className="lg:col-span-8">
          ...
          <div className="scroll-slim max-h-[248px] overflow-y-auto">
            ...
            {symRecords.slice(0, 12).map((r) => {
              const dist = ((r.price - r.detectedPrice) / r.detectedPrice) * 100;
              return (
                <div key={r.id}
                  className={`grid grid-cols-[64px_46px_1fr_60px_76px_118px_54px] items-center gap-2 border-b border-ink-700/25 px-4 py-[7px] transition-colors hover:bg-ink-750/50 ${r.isControl ? "opacity-60" : ""}`}>
                  <span className="font-mono text-[9.5px] text-mist-500">{fmtAgo(r.detectedAt, now)}</span>
                  <span className={`w-fit border px-1.5 py-0.5 font-mono text-[8px] font-bold uppercase ${
                    r.side === "long" ? "border-long-500/40 bg-long-900/50 text-long-300" : "border-short-500/40 bg-short-900/50 text-short-300"
                  }`}>
                    {r.side === "long" ? "↓ L" : "↑ S"}
                  </span>
                  <span className="tick-num font-mono text-[11px] font-semibold text-mist-200">
                    ...
                    {r.isControl && (
                      <span className="ml-2 border border-dashed border-mist-600 px-1 font-mono text-[7.5px] uppercase tracking-wider text-mist-500">control</span>
                    )}
                  </span>
                  <span className={`tick-num text-right font-mono text-[10px] ${dist < 0 ? "text-short-300" : "text-long-300"}`}>
                    {dist >= 0 ? "+" : ""}{dist.toFixed(2)}%
                  </span>
                  <span className="tick-num text-right font-mono text-[10px] text-mist-400">{r.isControl ? "—" : fmtUsd(r.sizeUsd)}</span>
          ...
          {/* backtest */}
          {bt && (
            <div className="anim-feed-in border-t border-ink-700/50 bg-ink-900/40 px-4 py-3">
              <div className="flex flex-wrap items-center gap-3">
                <span className="font-mono text-[9px] font-bold uppercase tracking-[0.18em] text-mist-400">Backtest histórico</span>
                <span className={`border px-2 py-1 font-mono text-[10px] font-bold uppercase tracking-widest ${verdictMeta[bt.verdict].c}`}>
                  {bt.verdict}
                </span>
                <span className="font-mono text-[9px] text-mist-600">{bt.pools} pools · {bt.controls} controles · {candles.length} velas</span>
              </div>
              <div className="mt-2 grid grid-cols-1 gap-2 sm:grid-cols-3">
                <div className="flex items-center gap-2" title="Tasa de barrido de los pools detectados">
                  <span className="w-24 shrink-0 font-mono text-[8px] uppercase tracking-wider text-mist-500">Barrido pools</span>
                  <MeterBar v={bt.hitRate} color="#2de0c0" height={6} />
                  <span className="tick-num w-10 shrink-0 text-right font-mono text-[9px] font-semibold text-mist-300">{Number.isFinite(bt.hitRate) ? `${(bt.hitRate * 100).toFixed(0)}%` : "—"}</span>
                </div>
                <div className="flex items-center gap-2" title="Tasa de barrido de los controles al azar (línea base)">
                  <span className="w-24 shrink-0 font-mono text-[8px] uppercase tracking-wider text-mist-500">Barrido azar</span>
                  <MeterBar v={bt.controlHitRate} color="#5f7396" height={6} />
                  <span className="tick-num w-10 shrink-0 text-right font-mono text-[9px] font-semibold text-mist-300">{Number.isFinite(bt.controlHitRate) ? `${(bt.controlHitRate * 100).toFixed(0)}%` : "—"}</span>
                </div>
                <div className="flex items-center gap-2" title="De los barridos, cuántos revirtieron después">
                  <span className="w-24 shrink-0 font-mono text-[8px] uppercase tracking-wider text-mist-500">Reversión</span>
                  <MeterBar v={bt.reversalRate} color="#ffb224" height={6} />
                  <span className="tick-num w-10 shrink-0 text-right font-mono text-[9px] font-semibold text-mist-300">{Number.isFinite(bt.reversalRate) ? `${(bt.reversalRate * 100).toFixed(0)}%` : "—"}</span>
                </div>
              </div>
              <p className="mt-2 font-mono text-[8.5px] leading-relaxed text-mist-600">{bt.note}</p>
            </div>
          )}
        </div>
      </div>
      ...
      <footer className="border-t border-ink-700/50 bg-ink-900/50 px-4 py-2.5">
        <p className="font-mono text-[9px] leading-relaxed text-mist-600">
          ...
          toca su nivel (tolerancia adaptativa a la distancia, máx. 0,12%); 15 min después se clasifica <b className="text-long-300">reversó</b> (rebotó,
          umbral adaptativo, máx. 0,4%) o <b className="text-short-300">atravesó</b>. Si la tasa de barrido de los pools supera a la de los{" "}
          <b className="text-mist-400">controles al azar</b>, el radar aporta señal real; si son iguales, es ruido. Los nocionales son
          estimados — lo que se valida aquí es el comportamiento de los niveles, no el monto.
        </p>
      </footer>
    </section>
  );
