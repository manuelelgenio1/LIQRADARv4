...
function TravelerDot({ from, to, color, dur }: { from: number; to: number; color: string; dur: number }) {
  ...
  return (
    <span className="pointer-events-none absolute left-[7px] z-10 h-[7px] w-[7px] -translate-x-1/2 -translate-y-1/2 rounded-full"
...
export default function MarketMakerPanel({ state, ind, cfg, confluence, market = "perp" }: Props) {
  ...
  const rel = Math.abs(target.price - cur) / span;
  const phase = rel < 0.12 ? 2 : rel < 0.35 ? 1 : 0;

  const rungs = state.clusters.slice(0, 7).map((c) => ({ id: c.id, price: c.price, side: c.side, sizeUsd: c.sizeUsd, leverage: c.leverage, isTarget: c.id === target.id, isSpot: false }));
  ...
  const topOf = (r: { id: string }) => layout.get(r.id) ?? 50;
  ...
  const factors = [
    { label: "Fuerza del clúster", v: target.strength, note: `Relativa al pico de calor (${Math.round(target.strength * 100)}%)` },
  ...
  const travelDur = 2 + rel * 6;

  return (
    <section className="panel panel-corner anim-reveal flex h-full flex-col" style={{ animationDelay: "0.2s" }}>
      <header className="flex items-center gap-3 border-b border-ink-700/50 px-4 py-3">
        ...
        <span key={up ? "u" : "d"} className="anim-feed-in ml-auto border px-2 py-1 font-mono text-[9px] font-bold uppercase tracking-widest"
      ...
      <div className="grid flex-1 grid-cols-[184px_1fr] gap-4 px-4 py-4">
        <div className="relative min-h-[240px]">
          ...
          <svg className="absolute inset-0 h-full w-full" preserveAspectRatio="none" viewBox="0 0 100 100">
            <line x1="7" y1={spotTop} x2="7" y2={targetTop} stroke={col} strokeWidth="0.8" strokeDasharray="2 2.5" vectorEffect="non-scaling-stroke"
              style={{ animation: "dashFlow 1.1s linear infinite", opacity: 0.9 }} />
          </svg>
          {spotRung && targetRung && <TravelerDot from={spotTop} to={targetTop} color={col} dur={travelDur} />}
          {rungs.map((r, i) => {
            const top = topOf(r);
            if (r.isSpot) {
              return (
                <div key={r.id} className="anim-feed-in absolute left-0 flex w-full items-center gap-2" style={{ top: `${top}%`, transform: "translateY(-50%)", animationDelay: `${i * 60}ms` }}>
                  <span className="relative h-[9px] w-[9px] shrink-0 rotate-45 border border-mist-200 bg-ink-900">
                    <span className="absolute inset-0 rotate-0 rounded-full border border-mist-200/50" style={{ animation: "pingSlow 2s ease-out infinite" }} />
                  </span>
            ...
            return (
              <div key={r.id} className="anim-feed-in absolute left-0 flex w-full items-center gap-2" style={{ top: `${top}%`, transform: "translateY(-50%)", animationDelay: `${i * 60}ms` }}>
                ...
                <div className="min-w-0 flex-1 border px-2 py-1 transition-colors hover:brightness-125"
        ...
        <div className="flex min-w-0 flex-col justify-center gap-2.5">
          {PHASES.map((p, i) => {
            ...
            return (
              <div key={p.n} className={`border px-3 py-2 transition-all duration-500 ${active ? "border-ink-600 bg-ink-800/80" : "border-ink-700/40 opacity-55"}`}
                ...
                <div className="flex items-center gap-2">
                  ...
                  {active && <span className="ml-auto h-1.5 w-1.5 rounded-full" style={{ background: col, animation: "liveBlink 1.4s ease-out infinite" }} />}
                </div>
          ...
          <div className="mt-1 border border-ink-700/50 bg-ink-900/50 px-3 py-2">
            ...
            {factors.map((f, i) => (
              <div key={f.label} className="anim-feed-in -mx-1 mb-1 flex items-center gap-2 rounded-sm px-1 transition-colors last:mb-0 hover:bg-ink-800/40"
                style={{ animationDelay: `${i * 80}ms` }} title={f.note}>
                <span className="w-[104px] shrink-0 truncate font-mono text-[8px] uppercase tracking-wider text-mist-500 transition-colors hover:text-mist-300">{f.label}</span>
